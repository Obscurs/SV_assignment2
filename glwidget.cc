...
void GLWidget::paintGL() {
  ...
  if (initialized_) {
    ...
    GLuint camera_location = program_->uniformLocation("camera_pos");
    glUniform3fv(camera_location, 1, cameraPos.data());

      GLuint phong_location = program_->uniformLocation("phong");
      glUniform1i(phong_location, phong_);

      GLuint skip_location = program_->uniformLocation("skip");
      glUniform1i(skip_location, skip_);
...
void GLWidget::SetSkip(bool set) {
    skip_ = set;
    updateGL();
}
